          ...
          @if (userMenuOpen()) {
            ...
            <div class="navbar__dropdown" role="menu" [attr.aria-label]="'nav.dropdown.user_menu_aria' | transloco" (click)="$event.stopPropagation()">
              ...
              @if (isAdmin() || isSuperAdmin()) {
                <hr class="navbar__dropdown-sep" role="none"/>
                @if (isAdmin()) {
                  <a routerLink="/admin/modules" class="navbar__dropdown-item" role="menuitem" (click)="userMenuOpen.set(false)">
                    ...
                    {{ 'nav.dropdown.admin_modules' | transloco }}
                  </a>
                  <a routerLink="/admin/users" class="navbar__dropdown-item" role="menuitem" (click)="userMenuOpen.set(false)">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>
                    {{ 'nav.dropdown.admin_users' | transloco }}
                  </a>
                }
                @if (isSuperAdmin()) {
                  <a routerLink="/superadmin/tenants" class="navbar__dropdown-item" role="menuitem" (click)="userMenuOpen.set(false)">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 21h18"/><path d="M5 21V7l8-4v18"/><path d="M19 21V11l-6-4"/></svg>
                    {{ 'nav.dropdown.superadmin_tenants' | transloco }}
                  </a>
                  <a routerLink="/superadmin/plans" class="navbar__dropdown-item" role="menuitem" (click)="userMenuOpen.set(false)">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 4h16v4H4z"/><path d="M4 12h16v8H4z"/></svg>
                    {{ 'nav.dropdown.superadmin_plans' | transloco }}
                  </a>
                }
              }
              <hr class="navbar__dropdown-sep" role="none"/>
...
export class NavbarComponent {
  ...
  /** Tenant admin — mirrors {@link adminGuard} (`/admin/*` routes). */
  readonly isAdmin = computed<boolean>(() => this.user()?.role === 'ROLE_ADMIN');
  /** Platform super admin — mirrors {@link superAdminGuard} (`/superadmin/*` routes). */
  readonly isSuperAdmin = computed<boolean>(() => this.user()?.role === 'ROLE_SUPER_ADMIN');
