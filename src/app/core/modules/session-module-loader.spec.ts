...
const FAKE_SESSION_PUBLIC_ROUTES: Routes = [{ path: 'join', loadComponent: () => Promise.resolve(class {}) }];
...
describe('loadSessionPublicModule', () => {
  afterEach(() => {
    vi.resetModules();
    vi.doUnmock('@pivot-platform/collaboratif-ui');
    provideCollaboratifUi.mockClear();
  ...
  it('resolves to a route wrapping SESSION_PUBLIC_ROUTES as children, when the package loads successfully', async () => {
    vi.doMock('@pivot-platform/collaboratif-ui', () => ({
      SESSION_PUBLIC_ROUTES: FAKE_SESSION_PUBLIC_ROUTES,
      provideCollaboratifUi,
    }));

    const { loadSessionPublicModule } = await import('./session-module-loader');
    const routes = await loadSessionPublicModule();

    expect(routes).toHaveLength(1);
    expect(routes[0].path).toBe('');
    expect(routes[0].children).toBe(FAKE_SESSION_PUBLIC_ROUTES);
  });
