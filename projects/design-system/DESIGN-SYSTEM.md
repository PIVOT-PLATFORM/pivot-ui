...
Fichiers (`projects/design-system/src/scss/`) : `tokens.scss` (variables), `reset.scss`,
`components.scss` (`.btn`, `.card`, `.form-*`, `.alert`, `.spinner`…), `patterns.scss`
(motifs `pv-*`). Chargés globalement par `src/styles.scss`.
