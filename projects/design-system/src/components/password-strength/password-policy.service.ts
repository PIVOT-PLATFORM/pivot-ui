...
/**
 * Token d'injection pour l'URL de l'API backend.
 * Fourni par l'application consommatrice — ne pas importer l'environment applicatif.
 *
 * @pivot/design-system EN17.8 — règle d'indépendance : pas d'import app-level.
 */
export const DESIGN_SYSTEM_API_URL = new InjectionToken<string>('DESIGN_SYSTEM_API_URL', {
  factory: () => '',
});
...
export class PasswordPolicyService {
  ...
  private readonly apiUrl = inject(DESIGN_SYSTEM_API_URL);
  ...
  load(): void {
    ...
    if (!this.apiUrl) return; // pas d'URL configurée — utiliser les défauts
