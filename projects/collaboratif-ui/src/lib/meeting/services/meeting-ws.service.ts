...
/** UI connection status for the STOMP link opened for a meeting's booking room (US12.4.1). */
...
/**
 * Minimal STOMP client wrapper for a single meeting's booking room (US12.4.1 "Temps réel") —
 * subscribes to `/topic/collaboratif/meeting/{id}` on the **shared** `collaboratif` real-time
 * endpoint (`/ws/whiteboard` — MeetOps has no dedicated endpoint of its own, mirroring
 * `SessionWsService`'s identical multiplexing over that one connection). Watch-only: every
 * mutating booking action (confirm, manual adjustment) is a plain REST call via
 * {@link import('./meeting-api.service').MeetingApiService}; the WS link exists solely to receive
 * broadcast state pushes so the validation screen's `aria-live` region can announce them
 * (US12.4.1 A11y AC).
 *
 * Authenticates once on `CONNECT` (never per-`SUBSCRIBE`), mirroring `SessionWsService`'s
 * bearer-on-CONNECT pattern — no guest-token fallback here: only an authenticated organizer/
 * participant can ever reach the validation screen (US12.4.1 "Sécurité — autorisation room
 * STOMP").
 *
 * Reconnection back-off relies on `RxStomp`'s own built-in retry behaviour, exactly like
 * `SessionWsService` (the template this mirrors) — this service does not set its own
 * `reconnectDelay`/`reconnectTimeMode`, deferring to whatever the injected {@link
...
export class MeetingWsService {
  ...
  /** Current connection status — surfaced by the validation screen's `aria-live` region. */
  ...
  /**
   * Connects and subscribes to the given meeting's booking topic. Safe to call once per mount;
   * call {@link disconnect} first to switch meetings on the same service instance.
   *
  ...
  /** `Authorization: Bearer <token>` for the authenticated organizer/participant. */
